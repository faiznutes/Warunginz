...
const getFallbackTemplate = (): ReceiptTemplate => ({
  id: 'default',
...
const applyTemplateDefaults = (candidate?: Partial<ReceiptTemplate> | null): ReceiptTemplate => ({
  ...getFallbackTemplate(),
  ...candidate,
  header: {
    ...getFallbackTemplate().header,
    ...(candidate?.header || {}),
  },
  footer: {
    ...getFallbackTemplate().footer,
    ...(candidate?.footer || {}),
  },
  fields: {
    ...getFallbackTemplate().fields,
    ...(candidate?.fields || {}),
  },
});
...
const hydrateReceiptData = () => {
  receiptData.value = props.receiptData || null;
  if (props.tenantName || props.tenantAddress || props.tenantPhone) {
    tenantInfo.value = {
      ...(tenantInfo.value || {}),
      name: props.tenantName || tenantInfo.value?.name || '',
      address: props.tenantAddress || tenantInfo.value?.address || '',
      phone: props.tenantPhone || tenantInfo.value?.phone || '',
    };
  }
};
...
const loadDefaultTemplate = async () => {
  try {
    ...
    template.value = applyTemplateDefaults(response.data);
  } catch (error: any) {
    console.warn('Receipt template fallback applied:', error?.response?.status || error?.message || error);
    template.value = applyTemplateDefaults();
  }
};
...
const loadReceiptData = async () => {
  ...
  try {
    ...
    if (!receiptData.value && props.orderId) {
      console.warn('Receipt data not preloaded for order:', props.orderId);
    }
  } finally {
    loading.value = false;
  }
...
const handlePrint = async () => {
  if (!receiptContent.value || !receiptData.value || !template.value) return;
  await printBrowser();
};
...
watch(() => props.receiptData, (newData) => {
  receiptData.value = newData || null;
});
