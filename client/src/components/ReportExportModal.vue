        ...
        <form @submit.prevent="handleExport" class="space-y-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <button
                v-if="availableExportModeValues.includes('analytics')"
                type="button"
              ...
              <button
                v-if="availableExportModeValues.includes('both')"
                type="button"
...
const availableReportTypeValues = computed(() => availableReportTypes.value.map((option) => option.value));
const availableExportModes = computed(() => getAvailableExportModes(currentRole.value));
const availableExportModeValues = computed(() => availableExportModes.value.map((option) => option.value));

const exportForm = ref({
  exportType: 'report' as 'report' | 'analytics' | 'both',
  reportType: (props.reportType as 'sales' | 'financial') || 'sales',
...
const ensureAllowedSelections = () => {
  if (!availableReportTypeValues.value.includes(exportForm.value.reportType)) {
    exportForm.value.reportType = availableReportTypeValues.value[0] || 'sales';
  }

  if (!availableExportModeValues.value.includes(exportForm.value.exportType)) {
    exportForm.value.exportType = availableExportModeValues.value[0] || 'report';
  }

  if (!supportsAnalyticsExports(currentRole.value) && exportForm.value.exportType !== 'report') {
...
const buildReportParams = () => {
  ...
  if (exportForm.value.period !== 'all') {
    params.startDate = exportForm.value.startDate;
    params.endDate = exportForm.value.endDate;
  }
  return params;
};
...
const fetchReportDataForExport = async () => {
  ...
  if (exportForm.value.reportType === 'financial') {
    if (!canAccessFinancialReports(currentRole.value)) {
      throw new Error('Laporan keuangan tidak tersedia untuk role ini.');
    }

    const response = await api.get('/finance/profit-loss', { params });
    return normalizeFinancialReportData(response.data);
  }
  ...
  return normalizeSalesReportData(summaryResponse.data, dailySalesResponse.data);
};

const fetchAnalyticsDataForExport = async () => {
...
const handleExport = async () => {
  ...
  try {
    ...
    if ((exportForm.value.exportType === 'report' || exportForm.value.exportType === 'both') && !reportData) {
      reportData = await fetchReportDataForExport();
    }

    if ((exportForm.value.exportType === 'analytics' || exportForm.value.exportType === 'both') && !analyticsData) {
      analyticsData = await fetchAnalyticsDataForExport();
    }
    ...
    if (exportForm.value.exportType !== 'report') {
      await showWarning('Export CSV hanya tersedia untuk laporan utama.');
      return;
    }

    const safeReportData = reportData || (await fetchReportDataForExport());
    let exportData: any[] = [];
    let headers: string[] = [];
    ...
    switch (exportForm.value.reportType) {
      case 'financial':
        exportData = [{
          Periode: exportForm.value.period,
          Pendapatan: safeReportData?.revenue || 0,
          'Biaya Pokok': safeReportData?.costOfGoods || 0,
          'Laba Kotor': safeReportData?.grossProfit || 0,
          'Margin Laba': `${Number(safeReportData?.profitMargin || 0).toFixed(2)}%`,
        }];
        headers = ['Periode', 'Pendapatan', 'Biaya Pokok', 'Laba Kotor', 'Margin Laba'];
        break;
      case 'sales':
      default:
        exportData = (Array.isArray(safeReportData?.byDate) ? safeReportData.byDate : []).map((item: any) => ({
          Tanggal: item?.date ? new Date(item.date).toLocaleDateString('id-ID') : '-',
          'Total Pendapatan': Number(item?.revenue || 0),
          'Jumlah Transaksi': Number(item?.count || 0),
          'Rata-rata': Number(item?.count || 0) > 0 ? Number(item?.revenue || 0) / Number(item?.count || 1) : 0,
        }));
...
watch(() => props.show, (newShow) => {
  if (newShow) {
    exportForm.value.reportType = (props.reportType as 'sales' | 'financial') || 'sales';
