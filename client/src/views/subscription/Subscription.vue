...
type SubscriptionViewModel = {
  ...
  activeAddons: Array<Record<string, unknown>>;
...
const deriveCountdownFromEndDate = (endDateValue: string | null | undefined) => {
  if (!endDateValue) {
    return {
      daysRemaining: 0,
      hoursRemaining: 0,
...
const normalizeSubscription = (raw: any): SubscriptionViewModel => {
  const subscriptionStart =
    raw?.subscription?.startDate ??
    raw?.subscriptionStart ??
    raw?.latestSubscription?.startDate ??
    null;
  const subscriptionEnd =
    raw?.subscription?.endDate ??
    raw?.subscriptionEnd ??
    raw?.latestSubscription?.endDate ??
    null;
  const countdown =
    raw?.daysRemaining !== undefined
      ? {
          daysRemaining: toNumberOrZero(raw.daysRemaining),
          hoursRemaining: toNumberOrZero(raw.hoursRemaining),
          minutesRemaining: toNumberOrZero(raw.minutesRemaining),
          secondsRemaining: toNumberOrZero(raw.secondsRemaining),
          isExpired: Boolean(raw?.subscriptionExpired ?? raw?.isExpired),
        }
      : deriveCountdownFromEndDate(subscriptionEnd);

  const effectivePlan = raw?.effectivePlan ?? raw?.plan ?? raw?.currentPlan ?? 'BASIC';
  const currentPlanId = raw?.currentPlan ?? raw?.plan ?? effectivePlan;
  ...
  return {
    ...
    effectivePlan,
    isExpired: raw?.subscriptionExpired ?? raw?.isExpired ?? countdown.isExpired,
    isActive: raw?.isActive,
    entitlements: Array.isArray(raw?.entitlements) ? raw.entitlements : [],
    limits: raw?.limits ?? null,
    activeAddons: Array.isArray(raw?.activeAddons) ? raw.activeAddons : [],
    latestSubscription: raw?.latestSubscription ?? null,
    subscription: {
...
const loadSubscription = async () => {
  ...
  try {
    ...
    stopCountdown();
    subscription.value = normalizeSubscription(response.data);
