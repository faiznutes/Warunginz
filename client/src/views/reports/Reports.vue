<template>
  <div class="flex flex-col gap-6 animate-fade-in font-display">
    ...
    <div v-if="showAnalyticsSection" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
    ...
    <div v-else-if="canAccessReportPage" class="flex flex-col items-center justify-center py-20 bg-white/50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
...
import {
  ...
  normalizeFinancialReportData,
  normalizeSalesReportData,
  shouldRestrictReportsToSales,
  supportsAnalyticsExports,
} from '../../utils/report-contracts';
...
const availableReportTypes = computed(() => getAvailableReportTypes(currentRole.value));
const canOpenExportModal = computed(() => {
  if (canExportReports.value) return true;
  return ['SUPER_ADMIN', 'ADMIN_TENANT', 'SUPERVISOR', 'CASHIER'].includes(currentRole.value || '');
});
const canAccessReportPage = computed(() => {
  if (canViewReports.value) return true;
  return ['SUPER_ADMIN', 'ADMIN_TENANT', 'SUPERVISOR', 'CASHIER'].includes(currentRole.value || '');
});
const needsTenantSelection = computed(() => authStore.isSuperAdmin && !authStore.selectedTenantId);
...
const reportType = ref<'sales' | 'financial'>('sales');
...
const showAnalyticsSection = computed(() => {
  return !!analyticsData.value && !loading.value && showAdvancedOptions.value && supportsAnalyticsExports(currentRole.value);
});
...
const ensureSupportedReportType = () => {
  const supportedTypes = new Set(availableReportTypes.value.map((option) => option.value));

  if (shouldRestrictReportsToSales(currentRole.value) && reportType.value !== 'sales') {
    reportType.value = 'sales';
  }

  if (!supportedTypes.has(reportType.value)) {
    reportType.value = 'sales';
  }
};

const handleReportTypeChange = () => {
  ensureSupportedReportType();
...
const reportRows = computed(() => {
  ...
  const byDateData = Array.isArray(reportData.value?.byDate) ? reportData.value.byDate : [];
  ...
  switch (reportType.value) {
    case 'sales':
      return safeMap(byDateData, (item: any, index: number) => {
        ...
        if (item?.products && Array.isArray(item.products)) {
          costOfGoods = item.products.reduce((sum: number, product: any) => sum + (product?.cost || 0), 0);
    ...
    case 'financial':
      return [[
        'Total',
        formatCurrency(reportData.value.revenue || 0),
        formatCurrency(reportData.value.costOfGoods || 0),
        formatCurrency(reportData.value.grossProfit || 0),
...
const loadSalesReport = async (params: Record<string, string>) => {
  const [summaryResponse, dailySalesResponse] = await Promise.all([
    api.get('/reports/dashboard/summary', { params }).catch(() => ({ data: null })),
  ...
  reportData.value = normalizeSalesReportData(summaryResponse.data, dailySalesResponse.data);
};

const loadFinancialReport = async (params: Record<string, string>) => {
  const response = await api.get('/finance/profit-loss', { params }).catch(() => ({ data: null }));
  reportData.value = normalizeFinancialReportData(response.data);
};
...
const loadReport = async () => {
  ...
  try {
    ...
    if (reportType.value === 'financial') {
      if (!canAccessFinancialReports(currentRole.value)) {
        reportType.value = 'sales';
...
const loadAnalytics = async () => {
  ...
  if (!supportsAnalyticsExports(currentRole.value)) {
    analyticsData.value = null;
    return;
  }

  const entitlements = await loadSubscriptionEntitlements();
  const canAccessAnalytics = authStore.isSuperAdmin || entitlements.includes('BUSINESS_ANALYTICS');
