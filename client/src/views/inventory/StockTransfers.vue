          ...
          <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700">
            ...
            <tr v-else v-for="transfer in transfers" :key="transfer.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm font-mono font-bold text-slate-900 dark:text-white">{{ transfer.transferNumber || transfer.id.substring(0, 8) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm text-slate-900 dark:text-white">{{ getStoreName(transfer.fromOutletId) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm text-slate-900 dark:text-white">{{ getStoreName(transfer.toOutletId) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm text-slate-900 dark:text-white">{{ getPrimaryProductName(transfer) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm font-bold text-slate-900 dark:text-white">{{ getTransferQuantity(transfer) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 border border-slate-200 dark:border-slate-700">
          ...
          <form @submit.prevent="createTransfer" class="space-y-4">
            ...
            <div>
              ...
              <input
                v-model.number="transferForm.items[0].quantity"
                type="number"
                min="1"
                required
                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              ...
              <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p class="mt-1 font-medium text-slate-900 dark:text-white">{{ getStoreName(selectedTransfer.fromOutletId) }}</p>
                  </div>
                  <div>
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">To Store</span>
                    <p class="mt-1 font-medium text-slate-900 dark:text-white">{{ getStoreName(selectedTransfer.toOutletId) }}</p>
                  </div>
                </div>
...
const toCollection = (input: any): any[] => {
  if (Array.isArray(input)) return input;
  if (Array.isArray(input?.data)) return input.data;
  return [];
};

const extractPagination = (input: any) => {
  if (input?.__pagination) return input.__pagination;
  if (input?.pagination) return input.pagination;
  return null;
};
...
const getStoreName = (outletId?: string | null) => {
  if (!outletId) return '-';
  return stores.value.find((store) => store.id === outletId)?.name || 'Unknown Store';
};

const getProductName = (productId?: string | null) => {
  if (!productId) return 'Unknown Product';
  return products.value.find((product) => product.id === productId)?.name || 'Unknown Product';
};

const getTransferQuantity = (transfer: any) => {
  if (!Array.isArray(transfer?.items)) {
    return Number(transfer?.quantity || 0);
  }
...
const getPrimaryProductName = (transfer: any) => {
  const firstItem = Array.isArray(transfer?.items) ? transfer.items[0] : null;
  if (!firstItem) return 'N/A';
  return firstItem.product?.name || getProductName(firstItem.productId);
};

const loadStores = async () => {
  try {
    const response = await api.get('/outlets');
    stores.value = toCollection(response.data);
  } catch (err) {
...
const loadProducts = async () => {
  try {
    ...
    products.value = toCollection(response.data);
  } catch (err) {
...
const loadTransfers = async (page: number = 1) => {
  ...
  try {
    ...
    if (filters.value.fromStore) params.fromOutletId = filters.value.fromStore;
    if (filters.value.toStore) params.toOutletId = filters.value.toStore;
    if (filters.value.status) params.status = filters.value.status;
    if (filters.value.date) params.date = filters.value.date;

    const response = await api.get('/stock-transfers', { params });
    transfers.value = toCollection(response.data);

    const nextPagination = extractPagination(response.data);
    if (nextPagination) {
      pagination.value = {
        page: nextPagination.page || page,
        limit: nextPagination.limit || pagination.value.limit,
      ...
      pagination.value = {
        ...
        totalPages: transfers.value.length > 0 ? 1 : 0,
      };
    }
  } catch (err: any) {
...
const createTransfer = async () => {
  try {
    if (transferForm.value.fromOutletId === transferForm.value.toOutletId) {
      await error('Source dan destination store harus berbeda');
      return;
    }
