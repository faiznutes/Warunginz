<template>
  ...
  <div v-else class="bg-slate-50 text-slate-900 font-display overflow-hidden h-screen w-screen flex flex-col relative">
    ...
    <ReceiptPrinter
      ...
      @close="handleReceiptPrinterClose"
      ref="printerRef"
    />
...
const printerRef = ref<{ handlePrint?: () => Promise<void> | void } | null>(null);
...
const advanceSplitAfterReceiptClose = ref(false);
...
const subscriptionEntitlements = ref<string[]>([]);
const subscriptionEntitlementsLoaded = ref(false);
...
const loadSubscriptionEntitlements = async (force = false) => {
  if (authStore.isSuperAdmin) {
    subscriptionEntitlements.value = ['BUSINESS_ANALYTICS', 'DELIVERY_MARKETING', 'DISCOUNTS'];
    subscriptionEntitlementsLoaded.value = true;
    return subscriptionEntitlements.value;
  }

  if (subscriptionEntitlementsLoaded.value && !force) {
    return subscriptionEntitlements.value;
  }
  ...
  try {
    ...
    subscriptionEntitlements.value = Array.isArray(payload.entitlements)
      ? payload.entitlements.map((feature: string) => String(feature).toUpperCase())
      : [];
  } catch (error: any) {
    console.error('Error loading subscription entitlements:', error);
...
const loadDiscounts = async () => {
  ...
  const canAccessDiscounts = authStore.isSuperAdmin || entitlements.includes('DISCOUNTS');
...
const isOfflineNetworkError = (error: any) => {
  return error?.code === 'ERR_NETWORK' || error?.code === 'ERR_INTERNET_DISCONNECTED';
};

const buildCheckoutPayload = (paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string }) => {
  const payload: any = {
    items: Array.isArray(cart.value)
      ? cart.value.map(item => ({
          productId: item.id,
  ...
  if (authStore.selectedStoreId) {
    payload.outletId = authStore.selectedStoreId;
  }

  if (customerType.value === 'customer' && customerName.value?.trim()) {
    payload.temporaryCustomerName = customerName.value.trim();
  }

  if (customerType.value === 'member' && selectedMember.value?.id) {
    payload.memberId = selectedMember.value.id;
  }

  if (paymentData.paymentMethod === 'CASH' && paymentData.cashAmount) {
    payload.cashAmount = Number(paymentData.cashAmount);
  }

  if (paymentData.qrCode) {
    payload.qrCode = paymentData.qrCode;
  }

  return payload;
};

const buildOfflineOrderData = (paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string }) => {
  const orderData = buildCheckoutPayload(paymentData);
  ...
  if (paymentData.paymentMethod === 'CASH' && paymentData.cashAmount) {
    transactionData.cashAmount = paymentData.cashAmount;
    transactionData.change = paymentData.cashAmount - finalTotal;
  }
...
const createReceiptData = (order: any, paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string }) => {
  const orderSubtotal = Number(order?.subtotal ?? subtotal.value);
  const orderDiscount = Number(order?.discount ?? discount.value ?? 0);
  const orderTotal = Number(order?.total ?? total.value);
  ...
  return {
    ...
    change:
      paymentData.paymentMethod === 'CASH' && paymentData.cashAmount
        ? paymentData.cashAmount - orderTotal
        : 0,
    servedBy: authStore.user?.name || 'Cashier',
  };
};

const emitKitchenOrderCreated = (order: any) => {
...
const checkoutOnline = async (paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string }) => {
  const response = await api.post('/orders/checkout', buildCheckoutPayload(paymentData));
  const payload = response.data?.data || response.data;
  ...
  estimatedDiscount.value = Number(payload.discount || discount.value || 0);
  lastOrderId.value = payload.id;
  emitKitchenOrderCreated(payload);
  return payload;
};
...
const finalizeSuccessfulCheckout = async (
  order: any,
  paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string },
  options: { showOverlay: boolean; closePaymentModal?: boolean },
) => {
  lastOrderReceipt.value = createReceiptData(order, paymentData);
  lastOrderId.value = order?.id || undefined;
  estimatedDiscount.value = Number(order?.discount || discount.value || 0);
  advanceSplitAfterReceiptClose.value = false;

  if (options.closePaymentModal) {
    showPaymentModal.value = false;
  }
...
const handleOfflinePaymentSaved = async () => {
...
const processPaymentSimple = async (paymentMethod: string) => {
  ...
  const paymentData: { paymentMethod: string; cashAmount?: number } = {
    paymentMethod,
  };
  ...
  try {
    ...
    if (isOnline.value) {
      try {
        order = await checkoutOnline(paymentData);
      } catch (error: any) {
    ...
    if (!isOnline.value || !order) {
      const fullOrderData = buildOfflineOrderData(paymentData);
      ...
      await handleOfflinePaymentSaved();
      return;
    }
    await finalizeSuccessfulCheckout(order, paymentData, { showOverlay: true });
  } catch (error: any) {
    if (!isOnline.value && isOfflineNetworkError(error)) {
      const fullOrderData = buildOfflineOrderData(paymentData);
      await offlineStorage.storeOrder(fullOrderData);
      await handleOfflinePaymentSaved();
    } else {
...
const processPayment = async (paymentData: { paymentMethod: string; cashAmount?: number; qrCode?: string }) => {
  ...
  try {
    ...
    await finalizeSuccessfulCheckout(order, paymentData, {
      showOverlay: true,
...
const handleReceiptPrinterClose = () => {
  showReceiptModal.value = false;
  if (!advanceSplitAfterReceiptClose.value) {
    return;
  }
...
const printReceiptAndClose = async () => {
  if (!lastOrderReceipt.value) {
    showError('Data struk tidak tersedia untuk transaksi ini.');
    return;
  }

  advanceSplitAfterReceiptClose.value = isSplitActive.value;
  showPaymentModal.value = false;
  showSuccessOverlay.value = false;
  ...
  const shouldAttemptAutoPrint = typeof navigator !== 'undefined' && !navigator.webdriver;
  if (!shouldAttemptAutoPrint) {
    return;
  }

  try {
    await printerRef.value?.handlePrint?.();
  } catch (error) {
    console.warn('Auto print receipt failed, manual print remains available.', error);
  }
};
