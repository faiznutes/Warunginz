...
const LOGIN_CACHE_ENABLED = process.env.PLAYWRIGHT_DISABLE_LOGIN_CACHE !== '1';
const loginPayloadCache = new Map<Role, Record<string, any>>();
let diskPayloadCache: Record<string, Record<string, any>> | null | undefined;
...
function getSharedAuthCachePath(): string | null {
  const candidate = process.env.PLAYWRIGHT_AUTH_CACHE_PATH;
  return typeof candidate === 'string' && candidate.trim().length > 0 ? candidate.trim() : null;
}

function readSharedLoginPayload(role: Role): Record<string, any> | null {
  const cachePath = getSharedAuthCachePath();
  if (!cachePath) return null;

  if (diskPayloadCache === undefined) {
    try {
      ...
      diskPayloadCache = null;
    }
  }

  const cached = diskPayloadCache?.[role];
  return cached && typeof cached === 'object' ? clonePayload(cached) : null;
}
...
export async function loginPayloadWithRetry(
  ...
  const sharedPayload = readSharedLoginPayload(role);
  if (sharedPayload) {
    if (LOGIN_CACHE_ENABLED) {
      loginPayloadCache.set(role, clonePayload(sharedPayload));
    }
    return clonePayload(sharedPayload);
  }

  if (LOGIN_CACHE_ENABLED && loginPayloadCache.has(role)) {
    return clonePayload(loginPayloadCache.get(role)!);
  }
  ...
  for (let attempt = 1; attempt <= LOGIN_MAX_ATTEMPTS; attempt += 1) {
    ...
    if (response.ok()) {
      const normalizedBody = (body || {}) as Record<string, any>;
      if (LOGIN_CACHE_ENABLED) {
        loginPayloadCache.set(role, clonePayload(normalizedBody));
      }
      return clonePayload(normalizedBody);
    }
