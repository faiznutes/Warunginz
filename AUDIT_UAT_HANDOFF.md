...
- Playwright critical multi-role: PASS (6/6)
...
| CASHIER | Tenant sendiri saja | assignedStoreId | /open-shift, /pos, /app/orders | open shift, transaksi POS, pembayaran, close shift | Tanpa shift aktif harus redirect ke /open-shift; tidak boleh aksi admin-level | flow shift harian deterministik dan store scope tidak bisa dibypass |
...
- `CASHIER`: wajib mengikuti `open shift -> POS -> close shift`; tanpa shift sehat harus terdorong ke gate shift.
