...
function startDetached(command, cwd, logPath) {
...
async function main() {
  ...
  run("npm run audit:uat:ready", "UAT readiness");

  const backendPid = backendRunning
    ? existing?.backendPid || null
    : startDetached(
        'node -r dotenv/config nest/dist/main.js dotenv_config_path=.env.local',
        ROOT,
        BACKEND_LOG,
      );
