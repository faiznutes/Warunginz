...
async function main() {
  const activeSubscriptionEnd = new Date();
  activeSubscriptionEnd.setDate(activeSubscriptionEnd.getDate() + 45);
  ...
  await prisma.tenantAddon.upsert({
    ...
    update: {
      ...
      limit: null,
      currentUsage: 0,
      expiresAt: activeSubscriptionEnd,
      purchasedBy: "ROLE_READINESS_SEED",
      config: null,
    },
    create: {
      tenantId: tenant.id,
