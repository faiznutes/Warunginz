...
const ROLE_CONTRACTS = [
  ...
  },
  {
    ...
    routes: "/open-shift, /pos, /app/orders",
    jobs: "open shift, transaksi POS, pembayaran, close shift",
    denies: "Tanpa shift aktif harus redirect ke /open-shift; tidak boleh aksi admin-level",
    evidence: "flow shift harian deterministik dan store scope tidak bisa dibypass",
  },
  {
    role: "KITCHEN",
    tenantScope: "Tenant sendiri saja",
...
function writeAuditReport() {
  ...
  lines.push("- CASHIER wajib melalui lifecycle shift (`open shift -> POS -> close shift`).");
...
function writeUatHandoff() {
  ...
  lines.push("- `CASHIER`: wajib mengikuti `open shift -> POS -> close shift`; tanpa shift sehat harus terdorong ke gate shift.");
...
function writeUatStartHere() {
  ...
  lines.push("- `CASHIER`: login, pastikan flow `open shift -> POS -> close shift` berjalan dan halaman cash shift tidak salah state.");
