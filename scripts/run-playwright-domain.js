...
const COMMAND = [
  ...
  "playwright",
  "test",
  "tests/playwright/domain-walkthrough.spec.ts",
  "--workers=1",
  "--reporter=line",
];
const ROLE_PATTERN = /\bfor (SUPER_ADMIN|ADMIN_TENANT|SUPERVISOR|CASHIER|KITCHEN)\b/;
const TEST_LINE_PATTERN = /domain-walkthrough\.spec\.ts:\d+:\d+\s+›\s+(.*)$/;
const FAILURE_LINE_PATTERN =
  /^\s*\d+\)\s+\[[^\]]+\]\s+›\s+tests\\playwright\\domain-walkthrough\.spec\.ts:\d+:\d+\s+›\s+(.*)$/gm;
