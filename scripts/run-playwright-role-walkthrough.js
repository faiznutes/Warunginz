...
const OUTPUT_JSON = path.join(ROOT, "AUDIT_PLAYWRIGHT_ROLE_WALKTHROUGH.json");
const OUTPUT_MD = path.join(ROOT, "AUDIT_PLAYWRIGHT_ROLE_WALKTHROUGH.md");
const COMMAND = [
  "npx",
  "playwright",
  "test",
  "tests/playwright/role-walkthrough.spec.ts",
  "--workers=1",
  "--reporter=line",
];
const ROLE_PATTERN = /\blive role walkthrough for (SUPER_ADMIN|ADMIN_TENANT|SUPERVISOR|CASHIER|KITCHEN)\b/;
const TEST_LINE_PATTERN = /role-walkthrough\.spec\.ts:\d+:\d+\s+›\s+(.*)$/;
const FAILURE_LINE_PATTERN =
  /^\s*\d+\)\s+\[[^\]]+\]\s+›\s+tests\\playwright\\role-walkthrough\.spec\.ts:\d+:\d+\s+›\s+(.*)$/gm;
...
function collectTitles(text) {
  ...
  for (const line of clean.split(/\r?\n/)) {
    ...
    if (!ROLE_PATTERN.test(title)) continue;
...
function writeMarkdown(report) {
  const lines = [];
  lines.push("# AUDIT PLAYWRIGHT ROLE WALKTHROUGH");
  lines.push("");
...
console.log(`Playwright role walkthrough report written: ${path.relative(ROOT, OUTPUT_MD)}`);
console.log(
  `Playwright role walkthrough summary: total=${summary.total}, passed=${summary.passed}, failed=${summary.failed}, skipped=${summary.skipped}`,
);
