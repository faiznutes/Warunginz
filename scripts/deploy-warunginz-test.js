...
const PUSH_ONLY = hasFlag("--push-only");
...
async function main() {
  ensureEnv();
  const summary = {
    deployMode: PUSH_ONLY ? "push-only" : DEPLOY_MODE,
  ...
  if (PUSH_ONLY) {
    ...
    if (summary.githubPush !== "passed") {
      throw new Error("Push-only mode failed to sync target repository.");
    }
    return;
  }
